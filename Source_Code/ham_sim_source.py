...
'''
def laserCoupling(photonicSpace:bool, ketbras, Omega,gLev,xLev,deltaL,args_list,pulseShape='np.sin(w*t)**2', ):
    def kb(x,y):
...
def couplingsToLaserHamiltonian(ketbras,atomStates, photonicSpace:bool,couplings, rabiFreq, pulseShape):
    hams, args_hams = [], dict()
    for xLev in couplings:
        ...
        if  xLev[1] in atomStates and xLev[2] in atomStates:
            ham, args_ham = laserCoupling(photonicSpace,ketbras, rabiFreq*xLev[0], xLev[1], xLev[2], xLev[3], args_hams, pulseShape) 
            hams.append(ham)
