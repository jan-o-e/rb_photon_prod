...
psi_init_3 = (
    ...
    / (np.sqrt(2))
    * (
        rb_atom_sim.kb_class.get_ket_atomic("g2PP", 0, 0)
        - rb_atom_sim.kb_class.get_ket("g2MM", 0, 0)
    )
)
psi_des_3 = (
...
psi_init = psi_init_3  #      CHANGE INITIAL STATE FOR THIS ROTATION HERE!
...
t = np.linspace(0, _length_repump_3, 10000)
...
print(f"fidelity after rotation 3 is {fidelity(psi_fin_3, rho_des)}")
...
t = np.linspace(0, _length_repump_4, 10000)
...
print(f"fidelity after rotation 4 is {fidelity(psi_fin_4, rho_des)}")
...
psi_init_5 = psi_fin_v4  #      CHANGE INITIAL STATE FOR THIS ROTATION HERE!
psi_des = params["psi_des"]
...
# run the simulation to find the final state density matrix
(output_states_list_5, t_list_5) = rb_atom_sim.run_repreparation(
    const_det_3,
    t,
    _delta_p,
    _delta_s,
...
psi_fin_5 = output_states_list_5[-1]

print(f"fidelity after rotation 3 is {fidelity(rho_fin, psi_fin_5)}")
...
#    ## ##   ##    ##    ##     ##  ##    ##  ##     ## ##           ##     ##
#     ###     ######     ##    #### ##     ## ##     ## ##            #######
psi_init_v5 = psi_fin_5

t = np.linspace(0, lengthStirap, 1000)

OmegaStirap_v5 = 70 * 2 * np.pi

delta_laser_v5 = -deltaZ + deltaZx2MM + delta_VST

laser_pulse_v5 = create_single_sinsquared(t, 1)

(output_states_list_v5, t_list_v5) = rb_atom_sim.run_vstirap(
    lengthStirap,
    OmegaStirap_v5,
    delta_laser_v5,
    "sigmaM",
...
psi_fin_v5 = output_states_list_v5[-1]
...
psi_init_6 = psi_fin_v5  #      CHANGE INITIAL STATE FOR THIS ROTATION HERE!
psi_des = params["psi_des"]
...
# run the simulation to find the final state density matrix
(output_states_list_6, t_list_6) = rb_atom_sim.run_repreparation(
    const_det_4,
    t,
    _delta_p,
    _delta_s,
...
(output_states_list_v6, t_list_v6) = rb_atom_sim.run_vstirap(
    lengthStirap,
    OmegaStirap_v4,
    delta_laser_v4,
    "sigmaP",
...
psi_fin_v6 = output_states_list_v6[-1]
...
t_list_full = combine_timelist(
    (t_list_3, t_list_v3, t_list_4, t_list_v4, t_list_3, t_list_v3, t_list_4, t_list_v4)
)
...
output_states_list_full = (
    ...
    + output_states_list_5
    + output_states_list_v5
    + output_states_list_6
    + output_states_list_v6
)

# save timelists individually and the concatenated one
with open(
    "/Users/ernst/Desktop/rb_photon_prod_dev/saved_data/full_sim_data/start_rot3/timelists.pkl",
    "wb",
) as f:
    pickle.dump(
        [
...
with open(
    "/Users/ernst/Desktop/rb_photon_prod_dev/saved_data/full_sim_data/start_rot3/density_matrix_evolution.pkl",
    "wb",
) as f:
    pickle.dump(
        [
