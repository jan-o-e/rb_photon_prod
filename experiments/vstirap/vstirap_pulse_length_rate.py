...
# length_list=[1.5,2]
area_list = [12.5, 25, 37.5, 50, 62.5, 75, 87.5, 100]
for area in area_list:
    ...
    for pulse_len in length_list:
        ...
        for H, t, args, label in H_list:
            ...
            output = mesolve(H, psi0_list[i], t, c_op_list, [], args=args, options=opts)
            ...
            if i == 0:
                scattering_rate = rb_atom.scattering_rate_of_transition(
                    atomStates,
                    "g1M",
                    "x2MM",
                    rb_atom.CGg1Mx2MM,
                ...
                print(
                    f"Scattering rate of the transition g1M -> x2MM is {scattering_rate}"
                )
            else:
                scattering_rate = rb_atom.scattering_rate_of_transition(
                    atomStates,
                ...
                print(
                    f"Scattering rate of the transition g1P -> x2PP is {scattering_rate}"
                )

            n_photon = plotter_cavemission(
                ketbras, atomStates, output.states, t, kappa, show_plts=False
            )
            print(f"Number of photons emitted is {n_photon}")

            total_scattering = rb_atom.plotter_spontdecay_channels(
                atomStates, output.states, t, show_plt=False
            )[1]
    ...
    with open(
        f"/Users/ernst/Desktop/rb_photon_prod_dev/saved_data/vstirap_length_rate/finer_sampling/data_lensweep_omegaL:{np.round(area/(2*np.pi),2)}.pkl",
        "wb",
    ) as f:
        pickle.dump(saved_processed_data, f)
    with open(
        f"/Users/ernst/Desktop/rb_photon_prod_dev/saved_data/vstirap_length_rate/finer_sampling/output_states_list_area:{np.round(area/(2*np.pi),2)}_l:{length_list[0]}_to{length_list[-1]}.pkl",
        "wb",
    ) as f:
        pickle.dump(output_states_list, f)
